 ...
 * ── PDFs ─────────────────────────────────────────────────────────────
 *
 * Deliberately NOT a branch inside `ingest()`. A photograph is processed
 * — rotated, stripped, resized into a derivative ladder; a PDF is
 * stored. Folding the two into one function would mean every step above
 * carrying an `if (isPdf) skip` around it, which is a worse read than a
 * second, shorter function that does only what a PDF needs:
 *
 *   original bytes
 *     → sniff the header, refuse anything that is not really a PDF
 *     → content-addressed key, uploaded AS IS — no sharp anywhere here
 *     → documents row, in one transaction
 *
 * No derivative ladder, no rotation, no LQIP: none of those concepts
 * apply to a PDF, and the published page embeds the original file
 * directly in an <iframe> — the reader's own browser (Firefox's built
 * in PDF.js, Chrome's, etc.) renders it, which is the entire viewer.
 */
...
/** `%PDF-` — the first five bytes of every valid PDF. Cheap and exact;
 *  no library needed to refuse a renamed .jpg before it reaches storage. */
...
export async function ingestDocument(
  ...
  if (!looksLikePdf(input.bytes)) {
    throw new IngestError("Not a readable PDF (the file's header does not start with %PDF-).");
  }

  const mime = "application/pdf";
  const digest = checksum(input.bytes);
  const key = contentKey(input.bytes, mime);
  ...
  const bytesStored = res.deduplicated ? 0 : input.bytes.byteLength;

  await client.query("BEGIN");
  try {
    const { rows } = await client.query<{ id: string; existed: boolean }>(
      `INSERT INTO documents
         (storage_key, provider, mime, bytes, title, source, source_ref,
          checksum, published, credit)
    ...
    return {
      ...
      bytesStored,
      deduplicated: rows[0]!.existed,
    };
  } catch (e) {
    await client.query("ROLLBACK");
    const err = e as { constraint?: string; message: string };
    if (err.constraint === "documents_title_check") {
      throw new IngestError(
        `Title rejected: "${input.title}". It must be at least 12 characters. ` +
          `Describe what the document is (e.g. its event, year, and kind).`,
      );
    }
    throw e;
  }
